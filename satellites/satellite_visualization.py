...
def draw_satellite_map(basemap_params, colored_attribute='orient'):
    ...
    # MAX_NUMBER = 100
    # SAMPLE_IDX = np.random.choice(range(17000), MAX_NUMBER)
    # all_satellites_names = np.array(list(satellites.keys()))[SAMPLE_IDX]
    # all_satellites = np.array(list(satellites.values()))[SAMPLE_IDX]
    ...
    all_geocentric = [sate.at(t) for sate in all_satellites] 
    ...
    all_velocity = [station.ITRF_position_velocity_error(t)[1] \
        for station in all_satellites]
    all_orient = [np.mod(np.rad2deg(np.angle(v[0] + v[1]*1j)) - 90, 360) \
        for v in all_velocity]
    if colored_attribute == 'orient':
        all_color_value = [sate.model.inclo for sate in all_satellites]
    elif colored_attribute == 'elevation':
    ...
    def update(frame):
        ...
        # sate_text = plt.text(x+8, y-8, all_satellites_names, color=[0.9,0.2,0.2])
