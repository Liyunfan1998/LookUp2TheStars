...
def celestial_sphere(colortype='Natural',file_name='BSC',mag=None):
    ## DATA
    # Generate random points w/ random RGB colors
    file_name_dict={'BSC':"./BSC.csv",'Hipparcos':'./hip_small.csv'}
    # 第一个：球面；第二个：底色
    color_dict={'Natural':[(0,0,0.3),(0,0,0)],'Greyscale':[(0.1,0.1,0.1),(0.6,0.6,0.5)],'Purple sky':[(0,0,0.2),(0.55,0.4,0.55)],
                'Printer greyscale':[(1,1,1),(0.2,0.2,0.2)],'Printer friendly':[(1,1,1),(1,1,0)]}
    #################################################################
    df=pd.read_csv(file_name_dict[file_name])
    ...
    rgb=list()
    for i in df['BV']:
        if colortype in ['Natural','Greyscale']:
            rgb.append(bv2rgb(i))
        elif colortype in ['Printer friendly','Printer greyscale']:
            rgb.append([0,0,0])
        elif colortype in ['Purple sky']:
    ...
    c_s_f=color_dict[colortype][0]  ########################################
    sphere_Actor.GetProperty().SetColor(c_s_f) #球面颜色（0-1）###############################
    ...
    # Background
    c_b=color_dict[colortype][1]###################################################
    renderer.SetBackground(c_b[0],c_b[1],c_b[2]) # 背景颜色（0-1）##################################
    # Reset camera
...
celestial_sphere(colortype='Purple sky',file_name='BSC',mag=4)