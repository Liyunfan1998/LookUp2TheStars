...
def celestial_sphere(colortype='Natural', file_name='BSC', mag=None):
    ...
    # 第一个：球面；第二个：底色
    color_dict = {'Natural': [(0, 0, 0.3), (0, 0, 0)], 'Greyscale': [(0.1, 0.1, 0.1), (0.6, 0.6, 0.5)],
                  'Purple_sky': [(0, 0, 0.2), (0.55, 0.4, 0.55)],
                  'Printer_greyscale': [(1, 1, 1), (0.2, 0.2, 0.2)], 'Printer_friendly': [(1, 0, 1), (0.5, 0.5, 0)]}
    #################################################################
    df = pd.read_csv(file_name_dict['BSC'])
    # db=pd.read_csv('./data/bound_small.csv')
    if mag:
        if mag < 4:
            mag = mag * 2
        elif mag == 4:
    ...
    for i in df['BV']:
        if colortype in ['Natural', 'Greyscale']:
            rgb.append(bv2rgb(i))
        elif colortype in ['Printer_friendly']:
            rgb.append([0, 255, 0])
        elif colortype in ['Printer_greyscale']:
            rgb.append([0, 0, 0])
        elif colortype in ['Purple_sky']:
